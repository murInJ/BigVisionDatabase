...
class SURF_Origin_Dataset(Dataset):
    def __init__(self,config="config.yaml",split='train'):
        super(SURF_Origin_Dataset, self).__init__()
        self.split = split
        self.root = config.get("SURF_root")
        self.protocol = config.get("SURF_train_protocol") if split == "train" else config.get("SURF_test_protocol")

        self.len = 0
        self.dir_list = []

        self.load_SURF(self.protocol)

    def __len__(self):
        return len(self.dir_list)

    def load_SURF(self, protocol):
        ...
        for idx in range(len(landmarks_frame)):
            ...
            video_name = video_name.replace('CASIA-SURF-CROP/', self.root)
